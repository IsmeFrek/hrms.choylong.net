...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (qTargetType === 'all' || qTargetType === 'hr' || qTargetType === 'user' || qTargetType === 'letter') setTargetType(qTargetType);
  ...
  const letterItems = items.filter((cr) => cr?.targetType === 'letter');

  const renderLetterTable = (list) => (
    <div className="overflow-x-auto bg-white border rounded">
      <table className="min-w-full text-sm">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-3 py-2 text-left">ល.រ</th>
            <th className="px-3 py-2 text-left">លេខលិខិត</th>
            <th className="px-3 py-2 text-left">ចំណងជើង</th>
            <th className="px-3 py-2 text-left">មន្ត្រីពាក់ព័ន្ធ</th>
            ...
            <th className="px-3 py-2 text-left">អ្នកស្នើសុំ</th>
            <th className="px-3 py-2 text-left">កាលបរិច្ឆេទ</th>
            <th className="px-3 py-2 text-left">Actions</th>
          </tr>
        </thead>
        <tbody>
          {list.map((cr, idx) => {
            const fields = cr?.payload?.fields || {};
            const attachments = cr?.payload?.attachments || [];
            return (
              <tr key={cr._id} className="border-t align-top">
                <td className="px-3 py-2">{idx + 1}</td>
                <td className="px-3 py-2">{fields.letterNo || '—'}</td>
                <td className="px-3 py-2 font-medium text-blue-800">{cr.reason || '—'}</td>
                <td className="px-3 py-2">
                  <div>{fields.officer || '—'}</div>
                  <div className="text-xs text-gray-500">{fields.officerId ? `ID: ${fields.officerId}` : ''}</div>
                  {fields.newRole && <div className="text-xs text-green-700 mt-1">តួនាទីថ្មី: {fields.newRole}</div>}
                  {fields.department && <div className="text-xs text-green-700">ផ្នែកថ្មី: {fields.department}</div>}
                </td>
                <td className="px-3 py-2">
                  {attachments.length ? (
                    <div className="flex flex-col gap-1">
                      {attachments.map((a, i) => (
                        <a key={i} href={a.url || a} target="_blank" rel="noreferrer" className="text-blue-600 underline">ឯកសារ {i+1}</a>
                      ))}
                    </div>
                  ) : '—'}
                </td>
                <td className="px-3 py-2">
                  {cr.status === 'approved' && <span className="px-2 py-1 text-xs rounded bg-green-100 text-green-800 border border-green-200">បានយល់ព្រម ហើយ HR នឹងកែប្រែទិន្នន័យតាម</span>}
                  {cr.status === 'rejected' && <span className="px-2 py-1 text-xs rounded bg-red-100 text-red-800 border border-red-200">បដិសេធ</span>}
                  {cr.status === 'pending' && <span className="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-800 border border-yellow-200">រង់ចាំ</span>}
                </td>
                ...
                <td className="px-3 py-2 flex items-center gap-2">
                  <button onClick={() => handleApprove(cr)} disabled={cr.status !== 'pending'} className={`px-2 py-1 rounded text-white ${cr.status === 'pending' ? 'bg-green-600' : 'bg-gray-400 cursor-not-allowed'}`}>អនុម័ត</button>
                  <button onClick={() => handleReject(cr)} disabled={cr.status !== 'pending'} className={`px-2 py-1 rounded text-white ${cr.status === 'pending' ? 'bg-red-600' : 'bg-gray-400 cursor-not-allowed'}`}>បដិសេធ</button>
                </td>
              </tr>
            );
          })}
    ...
    <div className="p-6">
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm text-gray-700 flex items-center gap-2">
            ...
            >
              ...
              <option value="letter">លិខិតបង្គាប់ការ</option>
            </select>
          </label>
          {targetType === 'user' && (
            <label className="text-sm text-gray-700 flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-semibold text-gray-900">លិខិតបង្គាប់ការ</div>
              <div className="text-sm text-gray-600">ចំនួន: {letterItems.length}</div>
            </div>
            {letterItems.length ? renderLetterTable(letterItems) : (
              <div className="bg-white border rounded p-4 text-sm text-gray-600">មិនមានសំណើលិខិតបង្គាប់ការ</div>
            )}
          </div>
        </div>
      ) : targetType === 'user' ? (
