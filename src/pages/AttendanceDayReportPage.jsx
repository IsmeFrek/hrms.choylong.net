...
function fmtKhmerLongDate(d) {
  ...
  const khMonths = ['មករា','កុម្ភៈ','មីនា','មេសា','ឧសភា','មិថុនា','កក្កដា','សីហา','កញ្ញា','តុលា','វិច្ឆិកា','ធ្នូ'];
...
export default function AttendancedayReportPage() {
  ...
  const columnMeta = useMemo(() => ({
    ...
    checkIn: { label: 'ម៉ោងចូល', width: 60, align: 'center', header: 'ចូល' },
    checkOut: { label: 'ម៉ោងចេញ', width: 60, align: 'center', header: 'ចេញ' },
  ...
  const defaultCols = useMemo(() => ([
    'index','name','gender','position','checkIn','checkOut','dayWorkCount','attendanceCount','leaveCount','leaveType','A','workTime',
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let monthlyRows = [];
        if (fromDate === toDate) {
          const dayRes = await api.get('/attendance/day-data', { params: { date: fromDate } }).catch(() => ({ data: [] }));
        ...
        monthlyRows.forEach((rec) => {
          ...
          // prefer direct checkIn/checkOut fields (returned by day-data)
          if (rec.checkIn || rec.checkOut) {
            if (rec.checkIn) target.checkIn = rec.checkIn;
            if (rec.checkOut) target.checkOut = rec.checkOut;
          }
          // extract check-in / check-out from monthly's dailyData when present
          try {
            const daily = Array.isArray(rec.dailyData) ? rec.dailyData : [];
            daily.forEach((entry) => {
              if (!entry || !entry.date) return;
              const key = new Date(entry.date).toISOString().slice(0, 10);
              if (key !== fromDate) return;
              const cin = entry.checkIn || entry.checkin || '';
              const cout = entry.checkOut || entry.checkout || entry.checkOutTime || '';
              const cinMin = parseHM(cin);
              const coutMin = parseHM(cout);
              if (cin && (!target.checkIn || (cinMin !== null && parseHM(target.checkIn) !== null && cinMin < parseHM(target.checkIn)))) {
                target.checkIn = cin;
              }
              if (cout && (!target.checkOut || (coutMin !== null && parseHM(target.checkOut) !== null && coutMin > parseHM(target.checkOut)))) {
                target.checkOut = cout;
              }
            });
          } catch (err) {
            // ignore parse errors
  ...
  const handlePrintByDepartment = () => {
    ...
    groupList.forEach(([deptName, rows]) => {
      ...
      if (fromDate === toDate) {
        header.appendChild(makeText('div', `សម្រាប់ ថ្ងៃទី: ${fmtKhmerLongDate(fromDate)}`, { marginTop: '4px', fontSize: '14px', textAlign: 'center', color: '#141313' }));
      } else {
  ...
  const renderCell = (key, row) => {
    switch (key) {
      ...
      case 'checkIn':
        return { value: row.checkIn || '', style: { textAlign: 'center', width: columnMeta.checkIn.width } };
      case 'checkOut':
        return { value: row.checkOut || '', style: { textAlign: 'center', width: columnMeta.checkOut.width } };
      case 'dayWorkCount':
    ...
    <div style={{padding:20, fontFamily:'"Khmer OS Siemreap","Noto Sans Khmer",Arial,sans-serif'}}>
      ...
      >
        ...
        <div style={{marginBottom:20, borderBottom:'2px solid #ddd', paddingBottom:10}}>
          ...
          <h1 style={{margin:0, fontSize:18, fontWeight:700, textAlign:'center'}}>វត្តមានប្រចាំថ្ងៃរបស់មន្រ្តីរាជការ និងមន្រ្តីកិច្ចសន្យា </h1>
          {fromDate === toDate ? (
            <p style={{margin:5, textAlign:'center', fontSize:16, color:'#111111'}}>
              សម្រាប់ ថ្ងៃទី: {fmtKhmerLongDate(fromDate)}
            </p>
          ) : (
            <p style={{margin:5, textAlign:'center', fontSize:16, color:'#111111'}}>
