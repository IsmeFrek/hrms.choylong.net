...
export default function PromotionByDiplomaReportPage() {
  ...
  const derived = useMemo(() => {
    const rows = (list || [])
      .filter(hr => (hr.salaryPromotionBy || '').trim() === 'សញ្ញាប័ត្រ' && hr.salaryPromotionDate)
      .filter(hr => {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `PromoDiploma_${year}${month?('_'+month):''}`);
    XLSX.writeFile(wb, `Promotion_Diploma_${year}${month?('_'+month):''}.xlsx`);
  };

  if (!perms.canViewHR) {
    return (
      <div className="p-6">
        <h2 className="text-xl font-semibold mb-2">របាយការណ៍ឡើងកាំប្រាក់តាមសញ្ញាបត្រ</h2>
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">របាយការណ៍ឡើងកាំប្រាក់តាមសញ្ញាបត្រ</h2>
      ...
      <div ref={printRef} className="bg-white p-4 border rounded print-scope">
        ...
        <div style={{textAlign:'center', marginBottom: '8px'}}>
          ...
          <div style={{fontFamily:'"Khmer OS Siemreap","Noto Serif Khmer", serif', fontSize:'13px',marginTop:'4px', fontWeight:600}}>បញ្ជីឈ្មោះមន្រ្តីរាជការឡើងកាំប្រាក់តាមសញ្ញាបត្រ ឆ្នាំ {toKhmerDigits(year)} {(month?(`ខែ ${toKhmerDigits(month)}`):'')}</div>
        </div>
        <table>
          <thead>
            <tr>
