...
export default function DateInput({ value, onChange, className = '', placeholder, disabled = false, min, max, disableWeekends = false, shortYear = false }) {
  ...
  placeholder={ph}
      ...
      <button
        ...
        disabled={disabled}
        className={`absolute inset-y-0 right-0 px-3 flex items-center text-purple-600 hover:text-purple-700 rounded-r-md ${disabled ? 'pointer-events-none opacity-60' : ''}`}
      ...
      {/* hidden native date input for calendar UI */}
      <input
        ref={hiddenRef}
        type="date"
