...
export default function SkillPage() {
  ...
  const onDragStart = (e, skill) => {
    console.log('dragstart', skill._id);
    setDraggingId(skill._id);
    // ensure browser starts drag operation by setting transfer data
    try {
      e.dataTransfer.setData('text/plain', skill._id);
    } catch (err) {
      // some browsers may throw if not allowed; ignore
    }
    e.dataTransfer.effectAllowed = 'move';
    // optional: give a smaller drag image so the whole row isn't used
  ...
  const onDragEnter = (e, skill) => {
    console.log('dragenter', skill._id);
    e.preventDefault();
    setDragOverId(skill._id);
  };
  const onDragOver = (e) => {
  ...
  const onDrop = async (e, targetSkill) => {
    console.log('drop target', targetSkill._id, 'dragging', draggingId);
    e.preventDefault();
    if (!draggingId) return;
    if (draggingId === targetSkill._id) {
      setDraggingId(null);
      setDragOverId(null);
      return;
    }
    setReordering(true);
    try {
      // swap contents of draggingId and targetSkill._id
      const a = skills.find(s => s._id === draggingId);
      const b = skills.find(s => s._id === targetSkill._id);
      if (!a || !b) return;
      const payloadA = {
        skills_Id: b.skills_Id,
        skills_Kh: b.skills_Kh,
        skills_En: b.skills_En,
        other: b.other,
      ...
      const payloadB = {
        skills_Id: a.skills_Id,
        skills_Kh: a.skills_Kh,
        skills_En: a.skills_En,
        other: a.other,
        total: a.total ?? a.Total,
        male: a.male ?? a.Male,
        female: a.female ?? a.Female
      };
      await Promise.all([
        skillAPI.updateSkill(a._id, payloadA),
        skillAPI.updateSkill(b._id, payloadB)
      ]);
      await fetchSkills();
    } catch (err) {
      console.error('Drag swap failed', err);
  ...
  const onDragEnd = () => {
    console.log('dragend', draggingId);
    setDraggingId(null);
    setDragOverId(null);
  };
  ...
  const openAddModal = () => {
    ...
    setNewID(''); setNewSkillKh(''); setNewSkillEn(''); setNewOther('');
  ...
  const openEditModal = (skill) => {
    setEditingRowId(skill._id);
    setEditID(skill.skills_Id);
    setEditSkillKh(skill.skills_Kh);
    setEditSkillEn(skill.skills_En);
    setEditOther(skill.other || '');
  ...
  const handleAddSkill = async () => {
    ...
    try {
      await skillAPI.createSkill({
        skills_Id: newID,
        skills_Kh: newSkillKh,
        skills_En: newSkillEn,
        other: newOther
      });
      setShowAddModal(false);
      fetchSkills();
  ...
  const handleUpdateSkill = async () => {
    if (!perms.canEditSkills) return;
    await skillAPI.updateSkill(editingRowId, {
      skills_Id: editID,
      skills_Kh: editSkillKh,
      skills_En: editSkillEn,
      other: editOther
    });
    setShowEditModal(false);
    setEditingRowId(null);
  ...
  const handleDeleteSkill = async (id) => {
    if (!perms.canEditSkills) return;
    if (window.confirm('លុបជំនាញនេះ?')) {
      await skillAPI.deleteSkill(id);
      fetchSkills();
    }
  };

  // Reorder skills by computing new ordering and persisting numeric skills_Id for all affected rows
  const moveSkill = async (skill, dir) => {
    if (!perms.canEditSkills) return;
    setReordering(true);
    try {
      const full = [...sortedSkills];
      const idx = full.findIndex(s => s._id === skill._id);
      if (idx === -1) return;
      const j = idx + dir;
  ...
  const handleExport = () => {
    const csvRows = [
      ['skills_Id', 'skills_Kh', 'skills_En', 'other'],
      ...skills.map(skill => [skill.skills_Id, skill.skills_Kh, skill.skills_En, skill.other])
    ];
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      rows.forEach(row => {
        const [skills_Id, skills_Kh, skills_En, other] = row.split(',').map(s => s.replace(/"/g, ''));
        if (skills_Id && skills_Kh && skills_En) {
          skillAPI.createSkill({ skills_Id, skills_Kh, skills_En, other });
        }
      });
      fetchSkills();
    };
  ...
  const sortedSkills = [...skills].sort((a, b) => {
    if (sortField === 'skills_Id') {
      const valA = Number(a.skills_Id) || 0;
      const valB = Number(b.skills_Id) || 0;
  ...
  return (
    <div className="p-6" style={{ fontFamily: "'Khmer OS Siemreap', 'Noto Sans Khmer', sans-serif" }}>
      ...
      <div className="mb-4 flex items-center">
        ...
        />
        <span className="mr-6 font-semibold">ចំនួនជំនាញ៖ {filteredSkills.length}</span>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg min-w-[350px]">
            <h3 className="text-lg font-bold mb-4 text-indigo-800">បន្ថែមជំនាញ</h3>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg min-w-[350px]">
            <h3 className="text-lg font-bold mb-4 text-indigo-800">កែប្រែជំនាញ</h3>
            <label className="block mb-1 font-medium text-lg text-blue-600">លេខសម្គាល់</label>
