...
const AttendanceSummarySchema = new mongoose.Schema({ staffId: String, year: Number, month: Number, attendanceCount: Number }, { strict: false });
...
async function main() {
  ...
  const allSummaries = await AttendanceSummary.find({}).lean();
  const summaries = allSummaries.filter(d => d.staffId && d.staffId.includes('\u200b'));
  ...
  for (const doc of summaries) {
    ...
    if (existing) {
      if ((doc.attendanceCount || 0) > (existing.attendanceCount || 0)) {
  ...
  const allMonthlies = await MonthlySummary.find({}).lean();
  const monthlies = allMonthlies.filter(d => d.staffId && d.staffId.includes('\u200b'));
  ...
  const allDailies = await AttendanceDailyReport.find({}).lean();
  const dailies = allDailies.filter(d => d.staffId && d.staffId.includes('\u200b'));
