...
async function run() {
  ...
  const hr = await mongoose.connection.collection('hrs').find({ khmerName: /ម៉េង/ }).toArray();
  console.log('HR Record:', JSON.stringify(hr.map(h => ({ name: h.khmerName, no: h.no, staffId: h.staffId })), null, 2));
