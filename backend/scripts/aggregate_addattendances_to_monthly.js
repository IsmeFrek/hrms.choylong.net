...
async function run() {
  ...
  for (const r of recs) {
    ...
    if (!map[sid]) map[sid] = { staffId: sid, name: r.staffName || r.staff?.fullName || r.name || '', dailyData: [], dayWorkCount: 0, attendanceCount: 0, totalMinutes: 0, absentCount: 0, leaveCount: 0 };
    ...
    const inMin = await parseHMtoMin(checkIn);
    const outMin = await parseHMtoMin(checkOut);
