...
const fileTransferOutSchema = new mongoose.Schema({
  ...
  timestamps: true,
  collection: 'filetransfers-out'   // <-- ប្រើ collection ផ្ទាល់
});

// Auto-increment the 'no' field
fileTransferOutSchema.pre('save', async function(next) {
  if (this.isNew) {
    try {
...
const FileTransferOut = mongoose.model('FileTransferOut', fileTransferOutSchema);
export default FileTransferOut;
